...
//            #return std::to_string(db->getLastInsertRowid());
    ...
    static void update(DB* db, ___BEAN___Bean bean)
    {
        try
        {
            ...
            SQLite::Statement query(*db, query_template.c_str());
    ...
    static void override(DB* db, ___BEAN___Bean bean)
    {
        try
        {
            SQLite::Statement query(*db, "UPDATE ___TABLE___ SET ___SET_OVERRIDE___ WHERE ___PK___ = ?");
...
def make_bean(bean_path, bean_filename):
    ...
    for i, bean_key in enumerate(fields):
        ...
        if is_optional:
            ...
            retrieve_stmt += 'bean.%s = query.getColumn(getIndex++).get%s();\n' % (bean_key, l[-1])
